...
class lfsr():
    def __init__(self, init, mask, length):
        self.init = init
        self.mask = mask
        self.lengthmask = 2**(length+1)-1 ''' max number '''

    def next(self):
        nextdata = (self.init << 1) & self.lengthmask ''' self.init << 1 '''
        i = self.init & self.mask & self.lengthmask  ''' self.mask or all zero '''
        output = 0
        while i != 0:     ''' output = i != 0 '''
            output ^= (i & 1)
            i = i >> 1
        nextdata ^= output ''' nextdata = output ? nextdata + 1 : nextdata'''
        self.init = nextdata
